...
class Wavery {
    constructor(s) {
        this.s = s;
        this.configuration = new index_js_1.WaveryConfiguration();
        this.normalizer = new index_js_1.Normalizer();
        this._N = 0;
    }
            ...
            if (index != null) {
                ...
                for (let term of sentence.split(" ")) {
                    ...
                    if (index[term] != null && !this.configuration.skipWord(term)) {
                        if (docIdsRetrieval == null) {
                            ...
                            docIdsRetrieval = utilities_js_1.intersect(docIdsRetrieval, new Set(index[term]));
                        }
                    }
                }
                ...
                if (docIdsRetrieval != null) {
                    for (var docId in docIdsRetrieval) {
                        let scorings = new index_js_1.Vector();
                        ...
                        findDocs.push(new index_js_1.Score(totalScore, docId, docIds[`${docId}`]));
                    }
                }
            }
