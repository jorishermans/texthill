...
export class TextileStore implements Store {
    ...
    async removeItem(key: string) {
        const oData: SearchModel[] = await this.repo.find({name: key});
        if (oData.length > 0) {
